...
static void delay_us( __IO uint32_t us)//1us
{
//	uint32_t i;
	// while(us--);
//	for(i=0;i<20;i++)
...
void DS1302_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_ResetBits(GPIOB, GPIO_Pin_0 | GPIO_Pin_2); //PB0，PB10置低
}
...
{ u8 time[15];
  ...
  time[14]=(time_buf[6]&0x0f);
	...
	calendar.w_year  =time[2]*10+time[3];
	calendar.w_month =time[4]*10+time[5];
	calendar.w_date  =time[6]*10+time[7];
	calendar.hour    =time[9]*10+time[10];
	calendar.min     =time[11]*10+time[12];
	calendar.sec     =time[13]*10+time[14];
}
