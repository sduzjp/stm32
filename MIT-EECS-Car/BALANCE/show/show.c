...
void oled_show(void)
{
	...
	if(Flag_Stop==0)OLED_ShowString(103,40,"O-N");
	if(Flag_Stop==1)OLED_ShowString(103,40,"OFF");
...
{    
	...
	if(app_3<0)app_3=-app_3;//对编码器数据就行数据处理便于图形化
	...
	printf("{B%d:%d:%d}$", (int)Voltage,Encoder_Left,Encoder_Right);//打印到APP上面 显示波形
}
/**************************************************************************
