...
u8 Flag_Way=0,Flag_Stop=1,Flag_Show=0,Flag_Next;//停止标志位和 显示标志位 默认停止 显示打开
int Encoder_Left,Encoder_Right;             //左右编码器的脉冲计数
int Encoder_A_EXTI,Flag_Direction;  
int Encoder_Temp;				//用于小车模式的确定
...
int main(void)
{ 
	Stm32_Clock_Init(9);         //=====系统时钟设置,倍频数为9，则系统时钟为72M
	delay_init(72);              //=====延时初始化，输入参数为系统时钟72M
